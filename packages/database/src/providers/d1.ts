 ...
 */
export type D1Config = D1ClientConfig
...
export class D1Client implements DatabaseClient {
  ...
  /**
   * Execute a raw D1 SQL query.
   * Prefer execute/first/all/batch outside the database package.
   */
  async queryRaw<T = unknown>(sql: string, params: unknown[] = []): Promise<D1QueryResponse<T>> {
    try {
      const response = await ofetch<D1QueryResponse<T>>(`${this.baseUrl}/query`, {
        method: 'POST',
        headers: {
  ...
  /**
   * Backward-compatible raw query entrypoint.
   * New business code should depend on DatabaseClient.execute instead.
   */
  async query<T = unknown>(sql: string, params: unknown[] = []): Promise<D1QueryResponse<T>> {
    return this.queryRaw<T>(sql, params)
  }

  /**
   * Execute a statement and return normalized mutation metadata.
   */
  async execute(sql: string, params: unknown[] = []): Promise<DatabaseExecuteResult> {
    const res = await this.queryRaw(sql, params)
    return this.mapMeta(res.result?.[0]?.meta)
  }
  ...
  async first<T = unknown>(sql: string, params: unknown[] = []): Promise<T | null> {
    const res = await this.queryRaw<T>(sql, params)
  ...
  async batch(statements: DatabaseStatement[]): Promise<DatabaseExecuteResult[]> {
    try {
      ...
      const response = await ofetch<D1QueryResponse>(`${this.baseUrl}/query`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${this.config.apiToken}`,
          'Content-Type': 'application/json'
      ...
      return response.result.map(result => this.mapMeta(result.meta))
    } catch (error: unknown) {
  ...
  private mapMeta(meta?: D1QueryResponse['result'][number]['meta']): DatabaseExecuteResult {
    return {
